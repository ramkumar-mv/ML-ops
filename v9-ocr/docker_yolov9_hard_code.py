...
SAVED_PLATES_DIR = '/app/saved_plates'
CLEAN_PLATES_DIR = '/app/clean_plates'
CAMERA1 = 'rtsp://admin:msfconsole1%24@192.168.30.27:554/onvif1'
CAMERA2 = 'rtsp://admin:msfconsole1%24@192.168.30.25:554/onvif1'
WEIGHTS = "/app/best.pt"
...
def process_image(file_path, detected_plates, seen_strings, output_dir):
    ...
    for char_img in segmented_chars:
        ...
        prediction = ocr_model.predict(img_array)  # Use ocr_model here
        predicted_label = class_labels[np.argmax(prediction)]
        predicted_string.append(predicted_label)
    
    predicted_string = ''.join(predicted_string)

    print(f"OCR Result: {predicted_string}")
    sys.stdout.flush()
    ...
    if is_valid_format(predicted_string) and is_valid_state_code(predicted_string):
        ...
        #print(f"Invalid plate format or state code: {predicted_string}")
        #sys.stdout.flush()
        os.remove(file_path)
...
def run_detection(weights, sources, data, imgsz=(320, 320), conf_thres=0.5, iou_thres=0.6, max_det=1000, device='', save_crop=True):
    print("Starting detection function")
    sys.stdout.flush()
    ...
    try:
        while True:
            for i, dataset in enumerate(datasets):
                ...
                for path, im, im0s, vid_cap, s in dataset:
                    ...
                    if frame_counts[i] % 100 == 0:
                        print(f"Processed {frame_counts[i]} frames from dataset {i+1}")
                        sys.stdout.flush()
            ...
            print(f'Completed a full cycle. Processed {sum(frame_counts)} frames in total.')
            sys.stdout.flush()
